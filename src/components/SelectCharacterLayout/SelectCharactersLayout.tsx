			...
			<div className="flex w-screen flex-col-reverse md:flex-row px-2">
				...
				<div className="flex gap-4 w-screen md:flex-wrap overflow-x-auto pb-2 ">
					{charactersData.results.map((character) => {
						...
						const characterHasNotChaptersInCommon = selectedCharacters.length === 1 && getIntersectionOfEpisodes([selectedCharacters[0], character])?.length === 0

						return (
							<CharacterCard
								key={character.id}
								character={character}
								disabled={selectedCharacters.length >= 2 || characterIsSelected || characterHasNotChaptersInCommon}
								action={async () => {
									"use server";
